...
export function AppShell({ sidebar, children }: { sidebar: React.ReactNode; children: React.ReactNode }) {
  ...
  return (
    <div className="group/shell min-h-screen bg-slate-100 text-slate-950" data-collapsed={collapsed}>
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-30 hidden border-r border-slate-200 bg-white p-4 shadow-sm transition-[width] duration-200 md:block",
          collapsed ? "w-20" : "w-64"
        )}
      >
        <button
          ...
          className="absolute right-3 top-3 inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 shadow-sm transition hover:bg-slate-50"
      ...
      <main className={cn("min-h-screen transition-[padding] duration-200", collapsed ? "md:pl-20" : "md:pl-64")}>
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur md:hidden">
          ...
          <form action={signOut}>
            <button className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700" title="Đăng xuất">
              <LogOut size={18} />
            </button>
          </form>
        </div>
